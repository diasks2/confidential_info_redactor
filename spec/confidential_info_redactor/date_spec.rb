...
RSpec.describe ConfidentialInfoRedactor::Date do
  context '#includes_date?' do
    it 'returns true if the string includes a date #001' do
      ...
      ws = ConfidentialInfoRedactor::Date.new(language: 'en')
  ...
  context '#replace' do
    context 'English (en)' do
      it 'replaces the date occurences in a string #001' do
        ...
        ws = ConfidentialInfoRedactor::Date.new(language: 'en')
        expect(ws.replace(string)).to eq('Today is  <redacted date> .')
      end

      it 'replaces the date occurences in a string #002' do
        string = 'The scavenger hunt ends on Dec. 31st, 2011.'
        ws = ConfidentialInfoRedactor::Date.new(language: 'en')
        expect(ws.replace(string)).to eq('The scavenger hunt ends on  <redacted date> .')
      end
    end
    ...
    context 'German (de)' do
      it 'replaces the date occurences in a string #001' do
        ...
        ws = ConfidentialInfoRedactor::Date.new(language: 'de')
        expect(ws.replace(string)).to eq(' <redacted date> ')
      end
  ...
  context '#replace_number_only_date' do
    it 'replaces only the number date occurences in a string' do
      ...
      ws = ConfidentialInfoRedactor::Date.new(language: 'en')
      expect(ws.replace_number_only_date(string)).to eq("Today is Tues. March 3rd, 2011.  <redacted date> ")
    end
  end
end
